...
- Agent: `RateLimiter` (middleware/rate_limit.py) — in-memory sliding-window per-session limiter (5 req/min, 20 req/hour). Keyed by `X-Session-Id` header with client-IP fallback. Returns 429 + `Retry-After`. Note: per-container state, so effective limit scales with warm Lambda count; good enough for dev/demo, Redis or API Gateway usage plans needed for true distributed limits.
...
### Changed
- Agent: `api_handler.py` now adapts FastAPI via `Mangum(app, lifespan="on")` — replaces the Wave 2 hand-rolled `rawPath` stub. Lifespan initialises the Neon pool, Anthropic client, compiled graph, budget tracker, and rate limiter once per cold start and reuses across warm invocations.
