...
### Fixed
- **Agent: cold-start crashed when Secrets Manager ARNs weren't resolved to plain env vars.** The agent Lambda's Terraform config ships `NEON_SECRET_ARN` + `ANTHROPIC_SECRET_ARN` (ARNs only, so IaC state doesn't leak credentials), but the lifespan read `NEON_DATABASE_URL` / `ANTHROPIC_API_KEY` directly from env. `init_langfuse` had its own Secrets Manager fetch; Neon + Anthropic did not — so `database_url` came back `None` (graph silently degraded to 503 on `/chat` and `/team`) and anything downstream that tried to touch Anthropic would fail. Symptom in the dashboard: `/team` 502 with zero log streams on `fpl-dev-team-fetcher` (the boto3 invoke never fires when the agent can't start properly), and `/chat` ending without a terminal SSE event. Added `_resolve_secret_to_env(arn_env, target_env)` in `services/agent/src/fpl_agent/api.py` that fetches the ARN from Secrets Manager and writes the plain value into env before `AsyncAnthropic()` is constructed and before the Neon pool is opened. Idempotent (skips when the plain var is already set, e.g. local dev or tests) and a missing ARN is tolerated (downstream callers choose fatality). Three unit tests cover the three paths.
