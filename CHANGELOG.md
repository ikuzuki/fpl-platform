...
### Added
- **Agent: write-once DynamoDB cache for raw FPL squad responses** in `services/agent/src/fpl_agent/squad_cache.py`, wired into `squad_loader.load_user_squad` via an optional `cache` parameter and a new `get_squad_cache` FastAPI dependency. Squad picks for a given `(team_id, gameweek)` pair are immutable once the GW deadline passes — the FPL `/entry/{id}/event/{gw}/picks/` endpoint returns the same payload forever after — so the cache needs no TTL and no invalidation. One cache row per `{team_id}#{gameweek}` key, JSON-encoded body. Read on `/team` before invoking the team-fetcher; on miss, invoke and write the body back on success (empty-picks responses are *not* cached so that a "never set" team can become non-empty after the user's first login). Beyond the latency + cost win (a cache hit is a single ~5 ms DynamoDB `GetItem` vs. a 500–2000 ms Lambda → FPL round trip), the cache also insulates the agent from intermittent Fastly IP blocks on the Lambda egress — once any squad has been fetched once, it stays reachable even while FPL's WAF is refusing AWS traffic. Cache read and write failures are logged and swallowed, so DynamoDB trouble always falls through to the live path. New Terraform table `aws_dynamodb_table.squad_cache` (`PAY_PER_REQUEST`, hash key `team_gameweek`), IAM `Resource` list on `lambda_agent_dynamo` extended to include it, `SQUAD_CACHE_TABLE` env var added to the agent Lambda. Eight unit tests cover the cache module; four more cover the loader integration (cache hit skips Lambda, cache miss writes after fetch, empty picks not cached, `cache=None` behaves exactly as before).
...
- **Agent: `GET /team` unwraps the `RunHandler` response envelope** in `services/agent/src/fpl_agent/squad_loader.py`. The team-fetcher Lambda returns `{"statusCode": N, "body": <payload>}` (the shape every `RunHandler`-wrapped Lambda produces, and exactly what API Gateway / Function URL proxy integrations expect), but the squad loader was reading `raw.get("picks", [])` directly off the envelope — which always returned `None`, so every valid team id collapsed into the "empty picks" branch and raised `SquadNotFoundError`. The route surfaced that as HTTP 404, CloudFront's distribution-wide `custom_error_response` for 404 swapped the body for `/index.html`, the dashboard tried to `response.json()` the HTML, failed, and rendered the generic "FPL is unreachable right now — try again shortly." error against every user's team id. This is the latent bug `/team` carried since #120 — it was masked the whole time by the infra-level 403 loop (#123 → #132 → the unblock PR that just landed), so no one reached the response-parse path until the Function URL was finally accepting requests. Fix peels off `raw["body"]`, branches on `statusCode != 200` as `SquadFetchError`, and removes the dead `FunctionError` branch (`RunHandler` catches every exception, so `FunctionError` is never set on the Lambda response metadata — the existing code could not have detected `TeamNotFoundError` via that path). A new regression test (`test_load_user_squad_unwraps_run_handler_envelope`) pins the envelope semantics. Follow-up: fine-grained 404 detection for genuine "team doesn't exist on FPL" — currently those surface as 502 because `str(TeamNotFoundError(<id>))` is just the integer ID, so the envelope's `body.error` string can't be distinguished from any other 500 cause; this needs either a `team_fetcher.main` catch-and-return-sentinel or a richer `RunHandler` error body.
- **Infra: unblocked the dashboard by reverting the agent Function URL to `AuthType = NONE` + `principal = "*"`, removing the agent CloudFront OAC, and pinning `LANGFUSE_TRACING_ENABLED = "false"` in Terraform.** Three coordinated changes across `infrastructure/environments/dev/agent.tf`, `infrastructure/modules/web-hosting/main.tf`, and `infrastructure/environments/dev/lambda.tf`. After #132 codified the `lambda:InvokeFunction` fix (which solved direct-curl to the Function URL), CloudFront→Function URL was *still* rejecting every origin request with 403 — metrics showed `Url4xxCount == UrlRequestCount`, so the October-2025 permission gap was never the only thing wrong with the OAC path. The second, separate issue is unresolved and parked as a follow-up (suspect RESPONSE_STREAM × OAC SigV4 signing interaction, or a SourceArn-condition mismatch on OAC-signed requests). Reverting to `NONE` + `"*"` is the same state that worked between #123 and #125; direct curl confirms the Function URL itself is healthy at that config. Separately, the post-#132 60-second hang on `/team` (and the matching "Stream ended before a final report" on `/chat`) was traced to Langfuse — with tracing enabled, the `@observe` decorator blocks the request path when the Langfuse endpoint is unreachable from this Lambda, so every request sat the full 60s timeout. Flipping `LANGFUSE_TRACING_ENABLED=false` via the CLI dropped `/team` from timeout to 493 ms; codifying it here stops the next `terraform apply` silently re-enabling the hang. Two follow-ups remain: (1) figure out why OAC-signed origin requests are rejected by the Function URL even with correct permissions, (2) fix Langfuse reachability/timeout so tracing can be safely re-enabled.
- **Infra: added missing `lambda:InvokeFunction` resource-policy statement AND restored the `AWS_IAM` + CloudFront-OAC hardening** in `infrastructure/environments/dev/agent.tf` and `infrastructure/modules/web-hosting/main.tf`. Root cause of every 403 the dashboard saw across the past several hours — an October-2025 AWS change now requires Function URLs to have **both** `lambda:InvokeFunctionUrl` AND `lambda:InvokeFunction` granted in the resource-based policy, checked independently. Our Terraform only had the first (it dated back to #123 when the Function URL was first stood up well before the cutoff); after the October change, every invocation returned `403 AccessDeniedException` "even when the function URL uses the NONE auth type" (per the Lambda docs). Explains the full incident chain: the #125 OAC hardening failed not because OAC was broken but because CloudFront's signed origin requests also needed `lambda:InvokeFunction`; the #130 revert to `AuthType=NONE` + `Principal=*` failed for the same reason; the #131 OAC removal was chasing a symptom, not the cause. Verified end-to-end: adding the second statement via `aws lambda add-permission` immediately flipped direct curl from `403` to `200 {"status":"ok"}`. This PR codifies the second statement AND reverses #130/#131 in the same apply — `AuthType` back to `AWS_IAM`, OAC re-created on the agent origin, both resource-policy statements scoped to `cloudfront.amazonaws.com` + the distribution ARN so the Function URL is once again only reachable via this distribution. Reference: https://docs.aws.amazon.com/lambda/latest/dg/urls-auth.html — "Starting in October 2025" note.
- **Infra: removed the Lambda OAC (`aws_cloudfront_origin_access_control.agent`) from the web-hosting module** and the corresponding `origin_access_control_id` reference from the `agent_api` CloudFront origin. Follow-up to the Function URL revert (#130): with `AuthType = NONE` + `principal = "*"` the Function URL ignores signatures entirely, so the OAC that #125 introduced is doing no useful work — and its presence was actively blocking Terraform apply. The OAC has a fixed name (`fpl-{env}-agent-oac`), so `terraform taint` tries to destroy the old one before the new one can be created; the distribution still references the old OAC, so `DeleteOriginAccessControl` fails with `OriginAccessControlInUse` and the apply errors mid-way (as happened on the taint attempt in the #130 incident — new Function URL created, distribution left pointing at the old deleted URL). Removing the OAC reference from the origin lets Terraform update the distribution, free the old OAC, and align state in a single apply. OAC will be reintroduced alongside a future re-hardening to `AuthType = AWS_IAM` once the root cause of the `#125`-era 403 loop is understood — tracked in `docs/architecture/security-architecture.md`.
