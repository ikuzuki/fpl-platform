...
### Fixed
- **Infra: added missing `lambda:InvokeFunction` permission on the agent Function URL** in `infrastructure/environments/dev/agent.tf`. Root cause of every 403 the dashboard has seen over the past several hours — an October-2025 AWS change now requires Function URLs to have **both** `lambda:InvokeFunctionUrl` AND `lambda:InvokeFunction` granted in the resource-based policy. Our Terraform had only the first statement (going back to #123 when the Function URL was first stood up pre-cutoff); after the October change, every invocation returns `403 AccessDeniedException` "even when the function URL uses the NONE auth type" (per the Lambda docs). Explains why earlier diagnostics were consistent with "account-level block on public Function URLs" — every public URL failed — while signed AWS_IAM requests from an IAM identity with `lambda:*` worked. Verified end-to-end: adding the second statement (`FunctionURLInvokeFunctionPublic`) via `aws lambda add-permission` immediately flipped direct curl from `403` to `200 {"status":"ok"}`. Reference: https://docs.aws.amazon.com/lambda/latest/dg/urls-auth.html — "Starting in October 2025" note.
