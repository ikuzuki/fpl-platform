...
## Revision — 2026-04-20: CloudFront-only enforcement via shared-secret header

The original decision leaves `authorization_type = "NONE"` + `principal = "*"` on the Function URL, so in principle anyone who discovers the `*.lambda-url.eu-west-2.on.aws` host can hit the Lambda directly — bypassing CloudFront, WAF, rate limits, the dashboard's CORS origin check, and observability. We tried hardening this with CloudFront Origin Access Control (OAC) + `authorization_type = "AWS_IAM"` in #125 / #132; it failed with 100% 4xx on every CloudFront-signed origin request.

**Why OAC is wrong for this endpoint.** OAC signs origin requests with SigV4 against the origin's service identity. For Lambda Function URLs, SigV4 over POST requires the *client* to compute `SHA256(body)` and send it as the `x-amz-content-sha256` header so the receiving service can verify payload integrity. CloudFront OAC does not hash bodies itself — it only relays whatever header the client sent. Browsers don't compute `x-amz-content-sha256`, so every `POST /chat` failed signature validation at the Function URL while `GET /team` (no body) would have worked. This is documented in an [AWS re:Post thread](https://repost.aws/questions/QUbHCI9AfyRdaUPCCo_3XKMQ) and the [arpadt OAC writeup](https://arpadt.com/articles/function-url-oac); confirmed in #133 after `Url4xxCount == UrlRequestCount` on the Lambda metrics.

**Chosen mitigation: shared-secret header.** CloudFront injects a high-entropy header (`X-CloudFront-Secret`) on every origin request to the agent. The FastAPI app validates the header via a middleware; missing or wrong → 401. The secret is generated by Terraform (`random_password` of length 48), stored in Secrets Manager for audit, and read into the Lambda at cold-start via the existing `resolve_secret_to_env` path. See `docs/architecture/security-architecture.md` for the full threat model and rotation procedure.

Security-equivalent to OAC for our threat model (direct-curl-to-Function-URL is rejected), works uniformly for GET and POST, and has no payload-hash constraint on the browser. Rejected alternatives and their reasons live in `docs/architecture/security-architecture.md`.
